...
## Null model
obj_nullmodel <- get(load("/GWAS/data/nov_gwas_staar_update/plt/snv_indel/step1_fitnullmodel/plt_variant_nullmodel_GENESIS.Rdata"))
...
## output path
output_path <- "/GWAS/data/nov_gwas_staar_update/plt/snv_indel/step3_gene_centric_coding_maf5percent/"
## output file name
output_file_name <- "plt_variant_coding"
...
for(kk in sub_seq_id)
{
	...
	results <- Gene_Centric_Coding(chr=chr,gene_name=gene_name,genofile=genofile,obj_nullmodel=obj_nullmodel,
	                               rare_maf_cutoff=0.05,rv_num_cutoff=2,category="all_categories_incl_ptv",
