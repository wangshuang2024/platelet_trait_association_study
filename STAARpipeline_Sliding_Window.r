...
jobs_num <- get(load("/GWAS/data/nov_gwas_staar_update/step0_preSTAAR/jobs_num.Rdata"))
## aGDS directory
...
## output path
output_path <- "/GWAS/data/nov_gwas_staar_update/plt/snv/snv_5mask/step6_sliding_window_maf5percent/"
## output file name
output_file_name <- "plt_snv_Sliding_Window"
...
for(kk in 1:(5e6/((sliding_window_length/2)*20)))
{
	...
	end_loc_sub <- end_loc + (sliding_window_length/2)*20*(kk-1) + (sliding_window_length/2)*20
	...
	{
		results <- try(Sliding_Window(chr=chr,start_loc=start_loc_sub,end_loc=end_loc_sub,sliding_window_length = sliding_window_length,genofile=genofile,obj_nullmodel=obj_nullmodel,type="multiple",rare_maf_cutoff=0.05,
